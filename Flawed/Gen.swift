...
public class BindName {
    ...
    public static func external(module: String, name: String) -> BindName {
        return BindName(kind: .external(module, name))
    }
}
...
extension BindName: PointerHashable {}
...
func genStat(
    ...
    case .block(let seq):
        ...
        let nameIndex = BindName.count
        for subStat in seq {
            try genStat(subStat, &subEnv, &inst)
        }
        for index in nameIndex ..< BindName.count {
            // What the hell...
            inst.append(.drop(BindName(kind: .anonymous(index))))
        }
...
func genExpr(
    ...
    case .identifier(let name):
        guard let bindName = env[name] else {
            throw GenerateError.unresolved(name, expr)
        }
        return bindName
